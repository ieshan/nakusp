...
// Nakusp2 is a background job processing system that supports multiple transport layers.
...
// NewNakusp2 creates a new Nakusp2 instance.
// It takes a configuration and a map of transports. If either is nil, it uses default values.
func NewNakusp2(config *models.Config, transports map[string]models.Transport) *Nakusp2 {
	if config == nil {
		config = &models.Config{
			MaxWorkers:         5,
...
// ID returns the unique identifier for the Nakusp2 worker instance.
func (n *Nakusp2) ID() string {
	return n.id
}

// AddHandler registers a handler for a given task name.
func (n *Nakusp2) AddHandler(taskName string, handler models.Handler) {
	n.lock.Lock()
	defer n.lock.Unlock()
...
func (n *Nakusp2) Publish(ctx context.Context, taskName string, payload string) error {
	n.lock.Lock()
	defer n.lock.Unlock()
...
func (n *Nakusp2) StartWorker(transportName string) error {
	n.lock.RLock()
...
// RunUntilCancelled is a helper function that runs a given handler function in a loop until the context is cancelled.
func (n *Nakusp2) RunUntilCancelled(ctx context.Context, handlerFn func(context.Context, string) error) {
	defer n.wg.Done()
...
func (n *Nakusp2) ExecuteJob(ctx context.Context, transport models.Transport, job *models.Job) error {
	n.lock.RLock()
